<flux:modal name="create-edit-customer" flyout position="bottom" variant="floating" @close="close">
    <form wire:submit="saveCustomer" class="space-y-4">
        ...
        <div>
            ...
            <flux:text class="text-gray-400 text-sm mt-1">
        ...
        <div class="grid grid-cols-2 gap-4">
            <flux:card class="space-y-6">
                    ...
                    <div class="w-72">
                        <label class="relative border-0 p-0 bg-transparent" label="Upload files">
                            ...
                            <div class="w-full aspect-3/2 overflow-hidden flex flex-col items-center justify-center rounded-lg border-dashed border-zinc-200 dark:border-white/10 border-2 bg-zinc-50 dark:bg-white/10 transition-colors in-data-dragging:bg-zinc-100 in-data-dragging:border-zinc-300 dark:in-data-dragging:bg-white/15 dark:in-data-dragging:border-white/20 [[disabled]_&amp;]:opacity-75 [[disabled]_&amp;]:pointer-events-none"
                                ...
                                @else
                                    <div class="flex flex-col items-center gap-2 py-4 px-8 ">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <flux:input wire:model="name" label="Full Name" placeholder="e.g. Pabitra Das" badge="Required"
                        required class="w-full" autocomplete="off" />
                ...
                <div class="grid grid-cols-2 gap-4">
                    <flux:input wire:model="mobile" label="Phone" placeholder="Phone number" badge="Required" required
                        autocomplete="off" />
            ...
            <flux:card class="space-y-6">
                <div>
                    ...
                    <flux:text class="mt-1">Enter the customer's address details.</flux:text>
                </div>

                <div class="grid grid-cols-2 gap-4">
                    <flux:select wire:model.live="selected_state" label="State" badge="Required" required
                    ...
                    <flux:select wire:model="selected_district" label="District" badge="Required" required
                ...
                <div class="grid grid-cols-2 gap-4">
                    <flux:input wire:model="city" label="City" placeholder="City" badge="Required" required
                        autocomplete="off" />

                    <flux:input type='number' wire:model="pincode" label="Pincode" placeholder="Pincode"
                        badge="Required" required autocomplete="off" />
                </div>

                <flux:textarea wire:model="address" label="Address" placeholder="Enter address here..." required />
            </flux:card>
        </div>
